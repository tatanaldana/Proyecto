$(document).ready(function() {
  ...
  $('#btn_venta').click(function() {
    ...
    $('#btnCalcular').click(function(event) {
        ...
        if (!usuarioData) {
            console.error('No se encontraron datos de usuario en el sessionStorage');
            return;
        }

        var usuarios = JSON.parse(usuarioData);
        if (usuarios.length === 0) {
            console.error('No se encontraron usuarios en el sessionStorage');
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    swal({
                        title: "Éxito",
                        text: "Venta ingresada de manera exitosa",
                        icon: "success",
                        buttons: {
                            confirm: "Aceptar",
                        },
...
$(document).ready(function() {
      ...
      $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success) {
                swal({
                    title: "Éxito",
                    text: "La venta se ha completado de manera exitosa",
                    icon: "success",
                    buttons: {
                        confirm: "Aceptar",
                    },
  ...
  //Cunado se da click en el boton con el id deletebutton< , se reucpera el valor doc_usuario del checkbox seleccionado
      var doc = $('input:checkbox:checked').data('idcarrito');
      console.log(doc);
  //Se realiza la petición AJAX por metodo POST 
      $.ajax({
        method: 'POST',
