...
//       var productoHTML = `
//           <div class="contenedor_productos row categoria" data-id_categoria="${producto.idProducto}">
...
function mostrarProductos(productos, idCategoria) {
  ...
  $.ajax({
    method: 'POST',
    url: 'carrito.php',
    // Utiliza el idProducto del objeto producto en la iteración actual
    data: { accion: 'agregar', idProducto: productos.idProducto },
    success: function(response) {
      if (Array.isArray(productos) && productos.length > 0) {
        contenedorTitulo.append(`<div class="imagenindex"><h1>${productos[0].nombre_cat}</h1></div>`);
        ...
        productos.forEach(function(producto) {
          var productoHTML = `
              <div class="contenedor_productos row categoria" data-id_categoria="${producto.idProductos}">
                  <form method="POST" action="../productos/carrito.php?accion=agregar&cod=${producto.cod}">
                      <div>
                          <div class="d-flex flex-column align-items-center">
                              <img src="../../public/img/categorias/catpollo.jpg" alt="${producto.nombre_pro}">
                              <div style="padding-top:20px;font-size:18px;">${producto.nombre_pro}</div>
                              <div style="padding-top:20px;font-size:18px;">${producto.detalle}</div>
                              <div style="padding-top:10px;font-size:20px;">${producto.cod}</div>
                              <div class="d-flex flex-column align-items-center">
                                  <input type="number" name="txtcantidad" value="1" size="1" class="mb-2" />
                                  <input type="submit" value="agregar" style="background: var(--primario); color: white; border:none; padding:10px; width:100%;" />
                                  <div class="contenido">${producto.detalle}</div>
    ...
    error: function(xhr, status, error) {
        console.log("No se encontró idProducto")
    }
  });
}
