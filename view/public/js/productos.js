    ...
    $('#btnbuscar').click(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                try {
                    //Se analiza la respuesta JSON obtenida del controlador y con la función json.parse 
                    //convertimos la cadena de texto JSON a un objeto javascript
                    var producto = JSON.parse(response);
                    if (!producto.error) {
                        //Si no hay errores en la respuesta, se almacena los datos de la consulta 
                        //en un 'sessionstorage', pero debemos convertir las valores de la consulta otravez 
                        //en una cadena json por medio de la funcion JSON.stringify
                        sessionStorage.setItem('productoData', JSON.stringify(producto));
                        console.log(producto);
  
                        // Redireccionamos al formulario de venta después de completar ambas solicitudes AJAX
                       window.location.href = '../../gestion/productos_adm.php';
                       } 
                                } catch (error) {
